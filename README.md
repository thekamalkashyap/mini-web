...
`src/` client (scenes are thin orchestrators; one controller per concern in
`game/input|world|pickups|fx|players|camera|debug|hud|views|boot`, reusable
React components in `ui/hud|ui/lobby`) · `shared/` simulation (no DOM deps;
`utils|config` foundation, `collision| soldier|sim` systems, `map|sim|soldier|
constants` kept as stable-import facades) · `server/` Colyseus (thin room +
`rooms/messages|sync`, `assets/` loaders) · `data|img|audio` extracted assets
(generated by `../mm_analysis` pipeline — do not hand-edit) · `test/`
headless suites. See `AGENT.md` for the working guide and golden rules.
