...
export class WorldScene extends Phaser.Scene {
  ...
  create() {
    try { this.createInner(); } catch (e) { window.__bootErr = (window.__bootErr || "") + "|world:" + (e && e.stack || e); console.error("world create failed", e); }
  }
  ...
  createInner() {
    ...
    this.remoteViews = new Map();/* sessionId -> {view, x, y, aim, tx, ty, taim} */
    this.pickupViews = [];
    this.vbullets = [];
    this.nades = [];
    ...
    this.sim = new WorldSim({
      mapJson: raw, maskData: this.registry.get("maskData"),
      frameSize: this.frameSizeCache, mode: o.mode === "online" ? "predict" : "solo",
    ...
    if (o.mode === "solo") {
      ...
      if (o.bots) for (const b of [["bot0", "BOT Diesel"], ["bot1", "BOT Chowdhury"]])
      ...
      this.pred = this.sim.addPlayer({
        id: "me", name: "YOU", cosmeticFire: true, authoritativeRespawn: true,
      });
      this.pred.invuln = 0;
    ...
    this.input.keyboard.on("keydown-M", () => { this.sfx.toggleMute(); this.toast("sound " + (this.sfx.muted ? "off" : "on")); });
    this.input.keyboard.on("keydown-BACKTICK", () => { this.showColliders = !this.showColliders; });
    this.input.mouse && this.input.mouse.disableContextMenu();
    this.input.on("pointerdown", p => { if (p.leftButtonDown()) this.lmb = true; if (p.rightButtonDown()) this.rmb = true; });
    this.input.on("pointerup", p => { if (!p.leftButtonDown()) this.lmb = false; if (!p.rightButtonDown()) this.rmb = false; });

    if (o.demo) this.demoHold = true;

    this.events.on("shutdown", () => this.cleanup());
    this._hudAcc = 0;
    this.toast("map: " + this.mapLabel + (o.mode === "online" ? " · online" : " · solo"));
  }

  /* ================================================================ room */
  wireRoom() { /* schema read directly in update(); events via ClientNet */ }

  /* ============================================================== build */
  buildBackground() {
    const name = this.map.tilesets[0].image.replace("tile64", "bg");   /* tile64Moon_new -> bgMoon_new */
    const key = this.textures.exists("bg:" + name) ? "bg:" + name : "bg:bg_new.png";
    const img = this.textures.get(key).getSourceImage();
    const bgS = this.scale.height * 1.25;
    const w = img.width * (bgS / img.height);
    this.bg = this.add.tileSprite(0, 0, this.scale.width, bgS, key)
      .setOrigin(0, 0).setScrollFactor(0).setDepth(-10);
    this.bg._tileScale = { x: w / img.width, y: bgS / img.height };
  }

  buildTilemap() {
    const tiled = this.make.tilemap({ key: "map" });
    const tsDef = this.map.tilesets[0];
    const tileset = tiled.addTilesetImage(tsDef.image.replace(/\.png$/, ""), "ts:" + tsDef.image);
    this.layerBg = tiled.createLayer("tilebg", tileset).setDepth(-5).setAlpha(0.9);
  ...
  buildObjects() {
    ...
    for (const ob of this.map.objects) {
      if (ob.name === "spritebg") this.add.image(ob.x, ob.y, "menu", (ob.props.sprite || "") + ".png").setDepth(-4).setAlpha(0.9);
  ...
  refreshPickupViews() {
    const src = this.opts.mode === "online" && this.room ? this.room.state.pickups : this.sim.pickups;
    for (let i = 0; i < this.pickupViews.length; i++) {
      const k = src[i];
      if (!k) continue;
      const respawn = k.respawn, idx = k.idx;
  ...
  buildFx() {
    ...
    this.emitters = {
      blood: this.add.particles(0, 0, "fx:blood", { speed: { min: 60, max: 260 }, angle: { min: 200, max: 340 }, scale: { start: 0.3, end: 0.1 }, lifespan: 600, gravityY: 600, emitting: false }).setDepth(6),
      spark: this.add.particles(0, 0, "fx:spark", { speed: { min: 80, max: 220 }, scale: { start: 0.2, end: 0.05 }, lifespan: 300, emitting: false }).setDepth(6),
      smoke: this.add.particles(0, 0, "fx:smoke", { speedY: { min: 40, max: 120 }, speedX: { min: -30, max: 30 }, scale: { start: 0.22, end: 0.4 }, alpha: { start: 0.8, end: 0 }, lifespan: 700, emitting: false }).setDepth(6),
    };
  ...
  updateInner(time, deltaMs) {
    ...
    if (this.opts.mode === "solo") {
      ...
      this._inpAcc -= dt;
      if (this._inpAcc <= 0) { this.net.sendInput(input); this._inpAcc = 1 / 30; }
    }
    ...
    if (this.showColliders) this.drawColliders(); else this.colliderGfx.clear();

    this._hudAcc -= dt;
    if (this._hudAcc <= 0) { this.syncHud(); this._hudAcc = 0.1; }
  }
  ...
  meReload() {
    ...
    else { this.net.sendReload(); this.pred.reload(); }
  }
  meNade() {
    if (this.opts.mode === "solo") this.sim.me.throwNade();
    else { this.net.sendNade(); this.predGhostNade(); }
  }
  predGhostNade() {
    ...
    const m = p.muzzle();
    this.nades.push({ x: m.x, y: m.y, vx: Math.cos(p.aim) * 780 + p.vx * 0.4, vy: Math.sin(p.aim) * 780 - 160, t: 0, fuse: 2, def: NADES.fragnade, ghost: true });
  }
  ...
  applyEvent(e) {
    ...
    switch (e.t) {
      case "fire":
        ...
        if (e.flame) { this.spawnBullet(e.x, e.y, e.vx, e.vy, e.life || 0.5, { flame: true }); }
        else {
          this.spawnBullet(e.x, e.y, e.vx, e.vy, e.life || 0.95, { rocket: e.rocket, ghost: true });
          this.sfx.play(e.sfx || "ak47", 0.45, rnd(0.95, 1.05));
        }
        break;
      case "nade": if (!solo) this.nades.push({ x: e.x, y: e.y, vx: e.vx, vy: e.vy, t: 0, fuse: 2, def: NADES.fragnade, ghost: true }); break;
      case "boom":
        this.spawnExplosion(e.x, e.y, e.r);
        this.cameras.main.shake(120, 0.006);
        this.sfx.play("explode", 0.8, rnd(0.9, 1.1));
        break;
      case "hit": this.emitters.blood.emitParticleAt(e.x, e.y, Math.min(12, e.n || 4)); break;
      case "spark": this.emitters.spark.emitParticleAt(e.x, e.y, (e.n || 2) * 3); break;
      case "smoke": this.emitters.smoke.emitParticleAt(e.x, e.y, 1); break;
      case "death":
        this.sfx.play(pick(["death1", "death3", "death5", "death9"]), 0.6);
        break;
      case "pk": /* pickup respawn sync comes via schema (online) / sim (solo) */ break;
      case "sfx": this.sfx.play(e.n, e.vol, e.rate); break;
      default: break;
    }
  }
  ...
  syncSoloViews(t, dt) {
    ...
    if (this.views) for (const [id, v] of this.views) if (!alive.has(id)) { v.destroy(); this.views.delete(id); }

    /* local bullets -> visuals (authoritative in solo) */
    for (const b of this.sim.bullets) this.spawnBullet(b.x, b.y, b.vx, b.vy, b.life, { flame: b.flame, rocket: b.rocket, tracked: b });
    /* nades drawn from sim state directly in stepNades when solo */
    if (this.opts.mode === "solo") {
      this.nades = this.sim.nades.map(n => ({ x: n.x, y: n.y, def: n.def, ghost: true, visual: true }));
    }
  }
  ...
  correctPrediction() {
    ...
    if (!s) return;
    const p = this.pred;
    if (s.hp < p.hp - 0.5) this.emitters.blood.emitParticleAt(p.cx(), p.cy(), 4);
    p.hp = s.hp; p.fuel = s.fuel; p.ammo = s.ammo; p.nades = s.nades;
    if (s.dead && !p.dead) p.die(null);
    if (!s.dead && p.dead) { p.dead = false; p.deadT = 0; p.invuln = 2.0; p.ensureBody(true); }
    const w = WEAPONS[s.wep] || WEAPONS.m61;
    if (w !== p.weapon) { p.weapon = w; p.weaponId = s.wep; this.sfx.play("switch", 0.4); }
    const dx = s.x - p.x, dy = s.y - p.y;
    if (Math.hypot(dx, dy) > 220) { p.x = s.x; p.y = s.y; }
    else { p.x += dx * 0.18; p.y += dy * 0.18; }
    if (p.body) Matter.Body.setPosition(p.body, { x: p.x + p.w / 2, y: p.y + p.h / 2 });
  }
  ...
  syncRemoteViews(dt, t) {
    ...
    state.players.forEach((s, id) => {
      ...
      if (id === this.room.sessionId) {
        ...
        if (!v) { v = new SoldierView(this); v.setSkin(this.pred.skin); (this.views || (this.views = new Map())).set(id, v); }
      ...
      r.tx = s.x; r.ty = s.y; r.taim = s.aim;
      r.state = s;
    ...
    for (const [id, r] of this.remoteViews) if (!seen.has(id)) { r.view.destroy(); this.remoteViews.delete(id); }
    if (this.views) for (const [id, v] of this.views) if (!seen.has(id)) { v.destroy(); this.views.delete(id); }
  }
  ...
  spawnBullet(x, y, vx, vy, life, o = {}) {
    if (o.tracked) {
      ...
      if (o.flame) { img.setTexture("menu", "flame1.png").setScale((o.tracked.r || 18) / 40).setAlpha(clamp(life * 3, 0, 1)); }
      else if (o.rocket) img.setTexture("menu", "rocket.png").setScale(0.4);
      else img.setTexture("fx:bullet").setScale(1).setAlpha(1);
      return;
    }
    this.vbullets.push({ x, y, vx, vy, life: life || 0.95, flame: o.flame, rocket: o.rocket, img: null });
  }

  stepVisualBullets(dt) {
    /* tracked sim-bullet images: mark live, destroy the ones whose bullet died */
    const live = new Set();
    for (const b of this.sim.bullets) if (b._img) { b._img.setVisible(true); live.add(b._img); }
    for (const img of this._bulletImgs) if (!live.has(img)) { img.destroy(); this._bulletImgs.delete(img); }
    /* ghost bullets (net events): integrate + draw */
    for (let i = this.vbullets.length - 1; i >= 0; i--) {
        const b = this.vbullets[i];
        b.life -= dt;
        if (b.life <= 0) { if (b.img) b.img.destroy(); this.vbullets.splice(i, 1); continue; }
        b.x += b.vx * dt; b.y += b.vy * dt;
        if (!b.img) {
          b.img = b.flame ? this.add.image(b.x, b.y, "menu", "flame1.png") : this.add.image(b.x, b.y, "fx:bullet").setDepth(6);
          b.img.setDepth(6);
        }
        b.img.setPosition(b.x, b.y).setRotation(Math.atan2(b.vy, b.vx));
        if (b.flame) b.img.setScale(rnd(14, 22) / 40).setAlpha(clamp(b.life * 3, 0, 1));
        else if (b.rocket) { b.img.setTexture("menu", "rocket.png").setScale(0.4); }
    }
  }
  ...
  stepNades(dt, t) {
    if (this.opts.mode === "solo") {
      ...
      for (const n of this.sim.nades) {
        if (!n._img) { n._img = this.add.image(n.x, n.y, "menu", n.def.sprite).setDepth(6).setScale(0.4); this._nadeImgs.add(n._img); }
        n._img.setPosition(n.x, n.y).setRotation(t * 6);
        live.add(n._img);
      }
      for (const img of this._nadeImgs) if (!live.has(img)) { img.destroy(); this._nadeImgs.delete(img); }
    } else {
      for (let i = this.nades.length - 1; i >= 0; i--) {
        ...
        if (n.t >= (n.fuse || 2)) { if (n.img) n.img.destroy(); this.nades.splice(i, 1); continue; }
        if (!n.img) n.img = this.add.image(n.x, n.y, "menu", n.def.sprite).setDepth(6).setScale(0.4);
  ...
  spawnExplosion(x, y, r) {
    const img = this.add.image(x, y, "menu", "explosion.png").setDepth(7).setScale(0.25).setAlpha(1);
    this.explosions.push(img);
    this.tweens.add({
      targets: img, scale: 1.45, alpha: 0, duration: 300, ease: "Quart.easeOut",
  ...
  stepFx(dt) {
    void dt;
    this.beamGfx.clear();
    if (this.sim) for (const bm of this.sim.beams) {
  ...
  updateCamera(dt, input) {
    ...
    if (!me) return;
    const zoomT = this.zoomOverride ? this.zoomOverride : (this.rmb && me.weapon.zoom ? 1.8 : 1);
    this.camZoom = lerp(this.camZoom, zoomT, 1 - Math.pow(0.001, dt));
    cam.setZoom(this.camZoom);
    const vw = this.scale.width / this.camZoom, vh = this.scale.height / this.camZoom;
    ...
    if (this.bg) this.bg.setTileScale(this.bg._tileScale.x, this.bg._tileScale.y).setTilePosition(-cam.scrollX * 0.3, -cam.scrollY * 0.15 + clamp((vh - vh * 1.25) / 2 + cam.scrollY * 0.1, -200, 0));
    void input;
  }
  ...
  syncHud() {
    ...
    useStore.getState().actions.setHud({
      ...
      dead: me.dead, deadT: Math.max(0, me.deadT), mapLabel: this.mapLabel,
  ...
  toast(msg) { useStore.getState().actions.toast(msg); }
