...
export class SoldierView {
  ...
  update(s, dt, time) {
    ...
    if (ground) {
      ...
      this.shadow.setFillStyle(0x000000, clamp(0.3 - d * 0.0009, 0.06, 0.3));
    ...
    set(this.body, skin.body, 0, -hip, 0.5, 0.95);
    set(this.head, skin.head, 1, -neck - 10, 0.5, 0.92);
    ...
    this.gun.setPosition(shoulderDx, -shoulder - 10);
    this.gun.setRotation(
    ...
    set(this.gunImg, gunName, -kick * 9, 2, 0.28, 0.7);
    ...
    if ((s.reloading || 0) > 0 && w.magFrame) {
      ...
      if (a > 0.03) {
        this.magImg.setTexture(ATLAS, w.magFrame);
        set(
          this.magImg,
          w.magFrame,
          gw * 0.36 - kick * 9,
    ...
    /* muzzle flash */
    this.flashT = Math.max(
      0,
      (s.flashT !== undefined ? s.flashT : this.flashT) - dt,
    );
    ...
    if (showFlash) {
      ...
      set(
        ...
        0.5,
        rnd(0, TAU),
        clamp(this.flashT * 22, 0, 1),
      );
      this.flashImg.setScale(SPR * (0.55 + Math.random() * 0.35));
    }
    set(this.armFront, skin.arm, gw * 0.08, 2, 0.4, 1, -0.05 - kick * 0.04);
