...
export class GameMap {
    ...
    for (let cy = 0; cy < rows; cy++)
      for (let cx = 0; cx < cols; cx++) {
        ...
        for (let dy = 0; dy < h; dy++)
          for (let dx = 0; dx < w; dx++) grid[cy + dy][cx + dx] = 0;
  ...
  surfaceProfile(step = 8) {
    const cols = Math.ceil(this.w / step),
      prof = new Float32Array(cols).fill(-1);
    if (!this.mask) {
      for (let cx = 0; cx < cols; cx++) {
        const tx = Math.floor((cx * step + step / 2) / this.tw);
        for (let ty = 0; ty < this.rows; ty++)
          if (this.gidAt(this.solid, tx, ty) > 0) {
            prof[cx] = ty * this.th;
            break;
          }
      }
    ...
    for (let cx = 0; cx < cols; cx++) {
      const mc = Math.min(
        this.maskCols - 1,
        ((cx * step + step / 2) / this.maskStep) | 0,
      );
      const mrows = (this.mask.length / this.maskCols) | 0;
      for (let r = 0; r < mrows; r++)
        if (this.mask[r * this.maskCols + mc]) {
          prof[cx] = r * this.maskStep;
          break;
        }
    }
    return { prof, step };
  ...
  surfaceSkins() {
    ...
    const emit = (r) => {
      ...
      const y0 = this.snapSurface(r.x0, r.y0),
        y1 = this.snapSurface(r.x1, r.y1);
    ...
    const ok = (r, x, y) => {
      ...
      if (r.pts)
        for (const p of r.pts) {
          if (
            Math.abs(cdx * (p.y - r.y0) - cdy * (p.x - r.x0)) / clen >
            MAX_DEV
          )
            return false;
        }
      ...
      if (run) {
        for (let k = 2; k <= 4 && !bridged; k++) {
          ...
          for (let s = 0; s < k - 1; s++) {
            ...
            if (sy < 0) continue;
            if ((cdx * (sy - run.y0) - cdy * (sx - run.x0)) / clen < -2) {
              dip = false;
              break;
            }
          }
