...
export class Soldier {
  constructor(sim, opts) {
    ...
    Object.assign(this, {
      ...
      shieldT: 0, jetOn: false, flaming: false,
      skin: { head: "head1.png", body: "body1.png", arm: "arm1.png", leg: "leg1.png" },
      kills: 0, ai: null, input: null,
  ...
  ensureBody(place) {
    ...
    if (!this.body) {
      this.body = Matter.Bodies.rectangle(this.x + this.w / 2, this.y + this.h / 2, this.w, this.h, {
  ...
  throwNade() {
    ...
    const n = { x: m.x, y: m.y, vx, vy, t: 0, fuse: 2.0, def: NADES.fragnade, owner: this };
    if (this.sim.engine) {
    ...
    this.sim.ev("nade", { x: Math.round(m.x), y: Math.round(m.y), vx: Math.round(vx), vy: Math.round(vy) });
  }
  ...
  fire(pressed, dt) {
    ...
    if (this.ammo <= 0) { this.sim.ev("sfx", { n: "dryfire", vol: 0.5 }); this.shootT = 0.3; this.reload(); return; }
    ...
    if (w.flame) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        this.sim.bullets.push({ x: m.x, y: m.y, vx, vy, dmg: w.dmg * (this.isBot ? 0.6 : 1), life: w.range / w.speed, owner: this, flame: true, r: rnd(14, 22) });
        this._fc = (this._fc || 0) + 1;
        if (this._fc % 3 === 0) this.sim.ev("fire", { flame: 1, x: Math.round(m.x), y: Math.round(m.y), vx: Math.round(vx), vy: Math.round(vy), life: 0.5 });
      }
      return;
    }
    ...
    if (w.beam) {
      ...
      for (const p of this.sim.players) if (p !== this && !p.dead && this.sim.segHitsRect(m.x, m.y, hit.x, hit.y, p)) p.damage(w.dmg * dmgMul, this);
      this.sim.ev("sfx", { n: "laser", vol: 0.55 });
      if (this.sim.map.solidAtPixel(hit.x, hit.y)) this.sim.ev("spark", { x: hit.x, y: hit.y, n: 3 });
    } else {
      for (let i = 0; i < (w.pellets || 1); i++) {
        ...
        this.sim.bullets.push({
          ...
          owner: this, rocket: w.rocket, splash: w.splash, arc: w.arc,
        });
        if (i === 0) this.sim.ev("fire", { x: Math.round(m.x), y: Math.round(m.y), vx: Math.round(vx), vy: Math.round(vy), rocket: w.rocket ? 1 : 0, sfx: fireSoundOf(w), life: w.rocket ? 3.2 : 0.95 });
      }
    }
  ...
  damage(d, by) {
    ...
    this.hp -= d;
    this.sim.ev("hit", { x: this.cx(), y: this.cy(), n: Math.min(8, 2 + d / 6) });
  ...
  die(by) {
    ...
    this.sim.ev("sfx", { n: pick(["death1", "death2", "death3", "death4", "death5", "death6", "death7", "death8", "death9"]), vol: 0.8 });
    if (by && by !== this) {
      by.kills++;
  ...
  physics(dt, input) {
    ...
    if (this.body) Matter.Body.setVelocity(this.body, { x: this.vx * dt, y: this.vy * dt });
    ...
    if (this.cosmeticFire) {
      if (input.fire && !this.dead && this.reloading <= 0 && this.shootT <= 0 && this.ammo > 0 && !this.weapon.melee && !this.weapon.flame) {
        this.flashT = this.sim.flashHold ? 0.5 : 0.05; this.recoilT = this.weapon.kick || 0.08; this.shootT = 60 / this.weapon.rpm;
      }
  ...
  postPhysics() {
    ...
    this.grounded = this.vy >= 0 && this.sim.map.rectHitsWorld(this.x + 2, this.y + this.h - 2, this.w - 4, 8);
    if (this.grounded) {
  ...
  update(dt, input) {
    ...
    const inp = input || (this.isBot ? (this.ai && this.ai.input) : this.input) || {};
    this.physics(dt, inp);
  }
}
...
export function botThink(b, dt) {
  ...
  const sim = b.sim;
  const target = sim.me && !sim.me.dead ? sim.me : sim.players.find(p => p !== b && !p.isBot) || sim.players.find(p => p !== b);
  if (ai.t <= 0) {
    ai.t = rnd(0.25, 0.5);
  ...
  if (target) {
    ...
    const los = !sim.segHitsSolid(b.cx(), b.shoulderY(), target.cx(), target.cy());
